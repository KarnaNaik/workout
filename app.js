...
const parseDuration = (str) => {
  ...
  if (lower.includes('rep')) return 45; // Default 45s for rep-based exercises
  return val;
};

const startQuickTimer = (name, seconds) => {
  workoutData.activeSession.routineKey = 'quick';
  workoutData.activeSession.exercises = [{ name, duration: seconds }];
...
const elements = {
  ...
  currentExerciseGifContainer: document.getElementById("currentExerciseGifContainer"),
  currentExerciseGif: document.getElementById("currentExerciseGif"),
  ...
  profileRest: document.getElementById("profileRest"),
  profileDarkMode: document.getElementById("profileDarkMode"),
...
const updateSessionUI = () => {
  const exercises = workoutData.activeSession.exercises;
  ...
  // Update Reference UI
  if (currentExercise && currentExercise.gif) {
    elements.currentExerciseGifContainer.classList.remove('hidden');
    elements.currentExerciseGif.src = currentExercise.gif;
  } else {
    elements.currentExerciseGifContainer.classList.add('hidden');
  }

  if (currentExercise && currentExercise.details) {
    elements.currentExerciseDetails.classList.remove('hidden');
    elements.currentExerciseDetails.textContent = currentExercise.details;
  } else {
    elements.currentExerciseDetails.classList.add('hidden');
  }
};
...
const updateProgressRing = () => {
  ...
  const total = workoutData.activeSession.phase === "rest" ? restDuration : currentExercise.duration;
...
const advancePhase = () => {
  ...
  if (workoutData.activeSession.phase === "exercise") {
    ...
    workoutData.activeSession.remaining = restDuration;
  } else {
    workoutData.activeSession.phase = "exercise";
    workoutData.activeSession.currentExerciseIndex += 1;
    const nextExercise = exercises[workoutData.activeSession.currentExerciseIndex];
    if (!nextExercise) {
      stopSession();
      markWorkoutComplete();
...
const startSession = () => {
  ...
  // Build exercises from the rich workoutLibrary to include GIFs and details
  const libraryData = workoutLibrary[routineKey];
    ...
    libraryData.sections.forEach(section => {
      section.exercises.forEach(ex => {
        sessionExercises.push({
          name: ex.name,
          duration: parseDuration(ex.duration) || 30,
          gif: ex.gif,
          details: ex.details
        });
      });
  ...
  workoutData.activeSession.exercises = sessionExercises;
  workoutData.activeSession.currentExerciseIndex = 0;
  workoutData.activeSession.phase = "exercise";
  workoutData.activeSession.isPaused = false;
  workoutData.activeSession.totalSeconds = 0;
  workoutData.activeSession.remaining = workoutData.activeSession.exercises[0].duration;
...
const adjustTimer = (seconds) => {
  ...
  if (workoutData.activeSession.phase === "exercise") {
    const exercises = workoutData.activeSession.exercises;
    const currentExercise = exercises[workoutData.activeSession.currentExerciseIndex];
    if (currentExercise) {
...
const loadProfile = () => {
  ...
  elements.profileRest.value = stored.restDuration || 15;
  elements.profileDarkMode.checked = stored.darkMode !== false;
  applyTheme(stored.darkMode !== false);
  restDuration = parseInt(stored.restDuration) || 15;
...
const renderProfileSummary = (profile) => {
  ...
  elements.profileSummary.innerHTML = `
    <div class="space-y-2">
      ...
      <p><span class="text-slate-400">Rest:</span> ${profile.restDuration || 15}s</p>
    </div>
  `;
};
...
const saveProfile = (event) => {
  ...
  const profile = {
    ...
    restDuration: elements.profileRest.value.trim(),
    darkMode: elements.profileDarkMode.checked
  };
  restDuration = parseInt(profile.restDuration) || 15;
...
init();
>>>>>>> 59de9e0505f584b4195beff3a707a50fdb39ff44
